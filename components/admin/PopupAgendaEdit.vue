          ...
          <div class="relative px-4 pb-4 mx-auto font-medium text-left">
            ...
            <div class="flex flex-col gap-2 mt-4">
              <div class="flex justify-between gap-4">
                <div class="w-full mt-4">
                  ...
                  <input v-model="selectedAgendaData.agenda_order" type="text" placeholder="Enter your Agenda Order here"
                    class="w-full px-4 py-2 text-xs rounded-lg border-primary-3" />
                </div>
                <div class="w-full mt-4">
                  <div class="text-sm">Start Time</div>
                  <input v-model="selectedAgendaData.agenda_start_time" type="text" placeholder="Enter your Agenda Start Time here"
                    class="w-full px-4 py-2 text-xs rounded-lg border-primary-3" />
                </div>
                <div class="w-full mt-4">
                  <div class="text-sm">End Time</div>
                  <input v-model="selectedAgendaData.agenda_end_time" type="text" placeholder="Enter your Agenda End Time here"
                    class="w-full px-4 py-2 text-xs rounded-lg border-primary-3" />
                </div>
              </div>
              ...
              <div class="w-full mt-4">
                ...
                <input v-model="selectedAgendaData.agenda_name" type="text" placeholder="Enter your Agenda Name here"
                  class="w-full px-4 py-2 text-xs rounded-lg border-primary-3" />
              </div>
              <div class="w-full mt-4">
                <div class="text-sm">Description</div>
                <input v-model="selectedAgendaData.agenda_description" type="text" placeholder="Enter your Agenda Description here"
                  class="w-full px-4 py-2 text-xs rounded-lg border-primary-3" />
              </div>
              <div class="mt-4">
            ...
            <div class="flex justify-between mt-4">
              ...
              <button class="items-center gap-2 px-4 py-2 text-sm font-bold text-white bg-blue-400 rounded-lg"
                @click="saveAgenda()" :disabled="isSubmit">
                {{ isSubmit ? 'Saving...' : 'Save Agenda' }}
              </button>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async saveAgenda() {
      ...
      formData.append("agenda_order", this.selectedAgendaData.agenda_order);
      formData.append("agenda_name", this.selectedAgendaData.agenda_name);
      formData.append("agenda_description", this.selectedAgendaData.agenda_description);
      formData.append("agenda_start_time", this.selectedAgendaData.agenda_start_time);
      formData.append("agenda_end_time", this.selectedAgendaData.agenda_end_time);
      formData.append("agenda_type_id", this.selectedAgendaData.agenda_type_id);
      if (this.agenda_doc.length > 0) {
      ...
      const result = await fetch(config.public.baseURL + '/event-agenda/' + this.selectedAgendaData.id, {
        method: 'POST',
        body: formData
      }).then(res => res.json())
      this.$emit('save-data');
