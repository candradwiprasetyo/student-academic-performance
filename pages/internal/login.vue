    ...
    <div class="w-full h-screen md:flex">
      <div class="flex-1 content-center flex items-center px-8 md:px-0">
        <div class="w-full md:w-[296px] mx-auto font-medium py-8 md:py-0">
          ...
          <div v-if="true">
            <div class="text-xl md:text-2xl font-extrabold font-montserrat">Login to <br>Internal System</div>
      ...
      <!-- <div class="absolute bottom-0">
        <img
          src="~/assets/images/login-footer.png"
          class="mx-auto"
          alt="Footer"
        />
      </div> -->
  ...
  methods: {
    async login() {
      ...
      if (this.user.email === '' || this.user.password === '') {
        ...
        const { authenticateUserInternal } = useAuthStore(); // use auth store
        const { authenticated } = storeToRefs(useAuthStore()); 
        await authenticateUserInternal(this.user);

        if (authenticated.value) {
          this.$router.push('/internal/dashboard')
        } else {
