  ...
  <div>
    <div class="rounded-lg p-8 bg-white shadow">
      <div class="flex justify-between">
        <div class="font-black text-lg">News Page</div>
        <a href="news-detail/add">
      ...
      <div class="border-primary-4 rounded-lg mt-4">
        <table class="w-full">
          ...
          <tr class="text-2xs md:text-xs border-b-2 border-gray-100" v-for="data in data" :key="data.id" v-if="isLoading==false">
            ...
            <td class="py-3 md:py-2 px-1 md:px-4">{{ $dayjs(data.publish_date).format('D MMMM YYYY') }}</td>
            <td class="py-3 md:py-2 px-1 md:px-4">
              <div 
...
export default {
  ...
  methods: {
    ...
    async loadData(token) {
      const data = await fetch(config.public.baseURL + '/article?type_id=1&limit=1000', { 
        headers: {
