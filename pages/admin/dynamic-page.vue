  ...
  <div>
    <div class="rounded-lg p-8 bg-white shadow">
      <div class="flex justify-between">
        <div class="font-black text-lg">Dynamic Page</div>
        <a href="dynamic-page-detail/add">
      ...
      <div class="border-primary-4 rounded-lg mt-4">
        <table class="w-full">
          ...
          <tr class="text-2xs md:text-xs border-b-2 border-gray-100" v-for="data in data" :key="data.id" v-if="isLoading==false">
            ...
            <td class="py-3 md:py-2 px-1 md:px-4">{{ data.title }}</td>
            ...
            <td class="py-3 md:py-2 px-1 md:px-4 text-right">
              <a :href="`dynamic-page-detail/${data.id}`">
                <img
...
export default {
  ...
  methods: {
    ...
    async loadData(token) {
      const data = await fetch(config.public.baseURL + '/dynamic-page?limit=1000', { 
        headers: {
    ...
    async changeStatus() {
      ...
      await fetch(config.public.baseURL + '/dynamic-page/' + this.selectedStatus.id, {
        method: 'POST',
        headers: {
